...
private:
  ...
  uint8_t writeIndex()
  {
    ...
    if (wi >= (uint16_t)__maxSize__)
      wi -= (uint16_t)__maxSize__;
    return (uint8_t)wi;
  }
...
public:
  ...
  rg_element_t & operator[](uint8_t inIndex)
  {
    ...
    if (index >= (uint16_t)__maxSize__)
      index -= (uint16_t)__maxSize__;
  ...
  bool push(const rg_element_t *inElements, uint8_t length)
  {
    if (isFull())
      return false;
...
template <typename rg_element_t, uint8_t __maxSize__>
bool RingBuffer<rg_element_t, __maxSize__>::isFull()
{
  return mSize == __maxSize__;
}

template <typename rg_element_t, uint8_t __maxSize__>
bool RingBuffer<rg_element_t, __maxSize__>::isEmpty()
{
  return mSize == 0;
}

template <typename rg_element_t, uint8_t __maxSize__>
void RingBuffer<rg_element_t, __maxSize__>::clear()
{
  mSize = 0;
}

template <typename rg_element_t, uint8_t __maxSize__>
uint8_t RingBuffer<rg_element_t, __maxSize__>::size()
{
  return mSize;
}

template <typename rg_element_t, uint8_t __maxSize__>
uint8_t RingBuffer<rg_element_t, __maxSize__>::maxSize()
{
  return __maxSize__;
}
