...
void Command_Motors() {
	...
	if (action == 'C') {
		FlowSerialWrite(255);
		// FlowSerialWrite(std::min(1, DUALVNH5019_ENABLED) * 2 + ADAMOTORS_SHIELDSCOUNT * 4 + std::min(1, DKMOTOR_SHIELDSCOUNT) * 4 + std::min(1, L98NMOTORS_ENABLED) * 2 + std::min(1, MOTOMONSTER_ENABLED) * 2 + std::min(4, SHAKEITPWM_ENABLED_MOTORS) + std::min(4, SHAKEITPWMFANS_ENABLED_MOTORS));
		// Подстройска под мою систему.
		FlowSerialWrite(3);
		...
		FlowSerialPrint(shShakeitL298N.providerName().c_str());
		FlowSerialPrint(";");
#endif
